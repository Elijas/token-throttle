...
def _is_builtin_sync_redis_backend_builder(backend: object) -> bool:
    redis_backend_module = sys.modules.get(
        "token_throttle._limiter_backends._redis._sync_backend"
    )
    redis_builder_cls = getattr(redis_backend_module, "SyncRedisBackendBuilder", None)
...
def _raise_if_redis_cluster_backend(backend: object) -> None:
    if not _is_builtin_sync_redis_backend_builder(backend):
        return
...
class SyncRateLimiter:
    ...
    ) -> "_SyncReservationScope":
        """
        Acquire capacity for a ``with`` block, then refund the unused remainder.

        Returns a context manager over the acquire -> call -> refund cycle.
        Entering it waits for capacity like ``acquire_capacity`` and yields a
        handle whose ``reservation`` attribute is the ``CapacityReservation``
        and whose ``set_actual_usage(actual_usage)`` records what the request
        actually consumed. The returned scope is single-use: re-entering it,
        whether after it has exited or while it is still active, raises
        ``RuntimeError`` — call ``reserve`` again for another attempt.
    ...
    def set_max_capacity(
        ...
        self._validate_shared_model_family_config(model, limit_config)
        if limit_config.is_unlimited:
            # Sync matches async unlimited rejection semantics and fails
            # before backend lookup.
    ...
    def diagnose(self) -> RateLimiterDiagnostic:
        """
        ...
        for model_family, backend in snapshot.backends.items():
            ...
            try:
                ...
                if inspect.isawaitable(raw_result):
                    raise TypeError("sync backend introspect() returned awaitable")  # noqa: TRY301
    ...
    def _diagnostic_snapshot(self) -> LimiterSnapshot:
...
class _SyncReservationScope:
    """
    Context manager returned by ``SyncRateLimiter.reserve``.

    Enter acquires capacity and yields this object as the handle, exposing
    ``reservation`` and ``set_actual_usage``; exit refunds. See
    ``SyncRateLimiter.reserve`` for the full acquire/refund contract.
    """
    ...
    def __enter__(self) -> Self:
        ...
        self._entered = True
        self.reservation = self._limiter.acquire_capacity(
    ...
    def __exit__(self, exc_type, exc, tb) -> Literal[False]:
        ...
        if self._actual_usage_set:
            ...
            self._refund_actual_or_conservative_close(self._actual_usage, reservation)
            return False
        ...
        self._limiter.refund_capacity(reservation.get_usage(), reservation)
        warnings.warn(
            _RESERVE_FORGOT_ACTUAL_USAGE_WARNING,
            RuntimeWarning,
            stacklevel=2,
    ...
    def _refund_actual_or_conservative_close(
        self,
        actual_usage: Usage,
        reservation: CapacityReservation,
    ) -> None:
        ...
        try:
            self._limiter.refund_capacity(actual_usage, reservation)
        except BaseException as refund_error:
            if isinstance(
                refund_error, LIFECYCLE_CALLBACK_CRITICAL_EXCEPTIONS
            ) or _exception_group_contains_critical(
            ...
            self._try_refund_swallowing_noncritical(
                reservation.get_usage(), reservation
            )
            raise

    def _refund_without_masking(
        self,
        refund_usage: Usage,
        reservation: CapacityReservation,
        *,
