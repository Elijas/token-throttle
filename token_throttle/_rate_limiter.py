...
class RateLimiter(BaseRateLimiter):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    await self._clear_refund_state_if_pending(
                        rid,
                        forget_in_flight=(
    ...
    async def _clear_refund_state_if_pending(
        self,
        reservation_id: str,
        *,
...
class _ReservationScope:
    ...
    async def __aexit__(self, exc_type, exc, tb) -> Literal[False]:
        ...
        if exc is not None:
            if self._usage_on_error is not None:
                await self._refund_without_masking(
    ...
    async def _refund_without_masking(
        ...
        *,
        ...
        if await self._try_refund_swallowing_noncritical(refund_usage, reservation):
            return
        if conservative_usage is not None:
            await self._try_refund_swallowing_noncritical(
                conservative_usage, reservation
            )

    async def _try_refund_swallowing_noncritical(
        self,
        refund_usage: Usage,
        reservation: CapacityReservation,
    ) -> bool:
