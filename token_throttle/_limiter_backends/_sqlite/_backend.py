...
class SqliteBackend(RateLimiterBackend):
        ...
        *,
        ...
        try:
            ...
            if settled_successfully:
                ...
                if attempt.available and not attempt.result.replayed:
    ...
    async def consume_capacity(
        ...
        *,
        ...
        if result.replayed:
            await self._missing_state_buckets_callback(result.missing_state_events)
        ...
        try:
            while True:
                ...
                if attempt.available:
                    break
                await self._missing_state_buckets_callback(
                    tuple(
                        event
    ...
    async def _missing_state_buckets_callback(
        self, events: tuple[MissingStateEvent, ...]
    ) -> None:
        if not (
