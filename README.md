...
[![PyPI Version](https://img.shields.io/badge/v3.0.0-version?color=43cd0f&style=flat&label=pypi)](https://pypi.org/project/token-throttle)
...
```bash
pip install "token-throttle[redis,tiktoken]>=3.0.0,<3.1.0"   # OpenAI + Redis (recommended)
pip install "token-throttle[redis]>=3.0.0,<3.1.0"            # Any provider + Redis
pip install "token-throttle>=3.0.0,<3.1.0"                   # Any provider + in-memory
```
