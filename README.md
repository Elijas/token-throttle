...
[![PyPI Version](https://img.shields.io/badge/v1.4.1-version?color=43cd0f&style=flat&label=pypi)](https://pypi.org/project/token-throttle)
...
```bash
pip install "token-throttle[redis,tiktoken]>=1.4.1,<1.5.0"   # OpenAI + Redis (recommended)
pip install "token-throttle[redis]>=1.4.1,<1.5.0"            # Any provider + Redis
pip install "token-throttle>=1.4.1,<1.5.0"                   # Any provider + in-memory
```
