...
Invariants compared across backends:
  ...
  * with try-acquire, SQLite may reject executor or engine-lock contention
    immediately; other backends queue in-process try-acquires. Every backend
    must account for exactly the requests it admits and never over-admit.
...
def _record(request, admitted: int, timeout: float, *, in_process: bool) -> None:
    ...
    if timeout > 0 or (in_process and request.node.callspec.params["kind"] != "sqlite"):
        # A bounded wait must admit everyone capacity allows; so must an
        # in-process try-acquire on backends that queue sibling calls. SQLite
        # timeout=0 instead rejects an occupied executor or engine lock.
        assert admitted == EXPECTED_ADMITTED, (
            f"admitted {admitted}, expected {EXPECTED_ADMITTED} (timeout={timeout})"
        )
    elif in_process:
        assert admitted >= 1, "an initially idle backend must admit a caller"
