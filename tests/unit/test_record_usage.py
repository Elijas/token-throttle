"""Tests for RateLimiter.record_usage — async non-blocking metering."""
...
class TestRecordUsageCallsConsumeCapacity:
    """record_usage delegates to backend.consume_capacity (not await_for_capacity)."""
    ...
    async def test_record_usage_calls_consume_capacity(self):
        ...
        mock_backend.await_for_capacity.assert_not_awaited()
        assert reservation.model_family == "gpt-4"

    async def test_record_usage_returns_reservation_with_correct_usage(self):
...
class TestRecordUsageValidation:
    ...
    async def test_empty_model_name_raises(self):
        ...
        with pytest.raises(ValueError, match="model_name cannot be empty"):
            await limiter.record_usage({"tokens": 1, "requests": 1}, model="")
    ...
    async def test_mismatched_usage_keys_raises(self):
        ...
        with pytest.raises(ValueError, match="do not match quota keys"):
            await limiter.record_usage({"tokens": 1}, model="gpt-4")
...
class TestRecordUsageUnlimited:
    ...
    async def test_unlimited_with_empty_usage_returns_unlimited_reservation(self):
        builder, _ = make_mock_backend_builder()
    ...
    async def test_unlimited_with_nonempty_usage_returns_unlimited_reservation(self):
        builder, _ = make_mock_backend_builder()
        limiter = RateLimiter(make_unlimited_config(), backend=builder)
