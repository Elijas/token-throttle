...
class TestAcquireCapacityValidation:
    ...
    def test_usage_must_be_mapping(self):
        ...
        with pytest.raises(ValueError, match="usage must be a mapping"):
            limiter.acquire_capacity([], model="gpt-4")
    ...
    def test_same_model_cannot_change_model_family(self):
        builder, _ = make_mock_backend_builder()
...
class TestAcquireCapacityForRequestValidation:
    ...
    def test_extra_usage_must_be_mapping_or_none(self):
    ...
    def test_false_model_in_request_raises(self):
        builder, _ = make_mock_backend_builder()
        limiter = SyncRateLimiter(make_limited_config(), backend=builder)

        with pytest.raises(ValueError, match="model_name must be a string"):
            limiter.acquire_capacity_for_request(model=False)

    def test_empty_string_model_in_request_raises(self):
        builder, _ = make_mock_backend_builder()
        limiter = SyncRateLimiter(make_limited_config(), backend=builder)

        with pytest.raises(ValueError, match="model_name cannot be empty"):
            limiter.acquire_capacity_for_request(model="")

    def test_integer_model_in_request_raises(self):
        builder, _ = make_mock_backend_builder()
        limiter = SyncRateLimiter(make_limited_config(), backend=builder)

        with pytest.raises(ValueError, match="model_name must be a string"):
            limiter.acquire_capacity_for_request(model=0)
    ...
    def test_unlimited_config_with_extra_usage_returns_unlimited_reservation(self):
        builder, _ = make_mock_backend_builder()
        limiter = SyncRateLimiter(make_unlimited_config(), backend=builder)
    ...
    def test_unlimited_config_with_counter_and_extra_usage_accepts_new_keys(self):
        def fake_counter(**_kwargs):
            return {"tokens": 100.0, "requests": 1.0}
...
class TestExtractTotalTokensValidation:
    ...
    def test_huge_int_total_tokens_raises_value_error_not_overflow(self):
    ...
    def test_hostile_float_total_tokens_raises_value_error_not_raw(self):
