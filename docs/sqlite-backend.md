...
produces `TimeoutError` promptly. A finite positive acquire timeout covers
initialization, executor queues, engine locks, capacity waiting, and SQLite
write-lock waiting. With no acquire timeout,
temporary lock contention is retried as part of the ordinary wait loop.

An occupied async executor or engine lock also makes `timeout=0` fail
immediately, including contention from callers sharing the same backend object.
This differs from Memory and Redis, which queue in-process try-acquires.
Use a positive timeout when a SQLite caller should wait behind other local
operations. The timeout result alone does not distinguish contention from
